...
/**
 * linear_skip - searches for a value conated in a skip; assumes a
 * list with sorted value and a single skip layer with nodes at every index
...
skiplist_t *linear_skip(skiplist_t *list, int value)
{
	...
	while (stop && stop->next != stop->express)
		stop = stop->next;
	/* value potentially lies between tow express nodes */
	if (temp->express)
	{
		printf("Value checked at index [%lu] = [%i]\n",
			temp->express->index, temp->express->n);
